...
Spec-driven project, early implementation. The database schema is **complete and applied** to the hosted Supabase project — content tables, `entities.slug`, Croatian service slugs, and usage logging (`events` + `log_event`). It holds **nine cities and 55 providers**: Zagreb (17), Split (13), Rijeka (5), Velika Gorica (5), Slavonski Brod (4), Zadar (4), Osijek (3), Pula (2), Dubrovnik (2). It grew from the Split-only pilot on 2026-09-03 and was filled out in Split's okolica on 2026-09-07 — Trogir, Kaštela, Solin and Omiš had been claimed by `CATCHMENT` since the pilot with no provider of their own in the data.

**These counts were read off the live data on 2026-09-16 and replace a stale seven-cities/51-providers line.** Two changes are undocumented anywhere in the repo and should not be guessed at: **Slavonski Brod and Velika Gorica were added**, and **Zagreb, Rijeka and Zadar each lost rows** (20→17, 6→5, 5→4) rather than gaining them. `CATCHMENT` in `web/lib/copy.ts` does already carry entries for both new cities, so the rule that every city needs one held through the change. **Never infer these numbers** — `getCityCoverage` counts `entities` rows unfiltered, so the landing page is the cheapest honest check. Curated source data and the provenance trail for each city live in `data/` (gitignored); `data/SPLIT_OKOLICA_REVIEW.md` carries the Split research, its exclusions and the five decisions behind it. **`supabase/migrations/` is gitignored too, so `SPEC_database.md` → Migrations is the only record of a migration that exists in the repo** — a migration missing from that table has no record at all, which is how the entire 2026-09-03 set went undocumented until 2026-09-07.
...
**Croatian copy lives where it is used, and that is a decision.** No i18n layer, no central string dictionary — the product is single-locale, so a dictionary would buy translation we will never need while making the markup unreadable. Copy is reviewed by reading the page files; a generated review document was tried on 2026-09-07 and removed the same day as more machinery than the job needs.

**The specs must not hold customer-facing text.** The code is the source of truth for copy; a spec records the *decision* about a piece of copy and quotes at most a short phrase where the decision is unintelligible without it. A paragraph pasted into `SPEC_frontend.md` becomes a silent second version the moment the page is edited — which is exactly what happened, and what the 2026-09-07 cleanup removed.

**No exact provider count goes in customer-facing prose** (owner decision, 2026-09-07). Use `providerShare` for a worded proportion (*"većina pogrebnika"*, *"gotovo svaki pogrebnik"*) or `providerFloor` for a scale claim that rounds down to the previous ten (*"više od 50"*) — both in `web/lib/copy.ts`, both derived from live rows, so the wording still moves with the data. The reasoning: a sentence built from two live counts has to be re-read every time the data changes, and the proportion was the fact the reader wanted while the count made them do arithmetic first. Five counting helpers went dead with that change and were deleted (`providerCount`, `providerNoun`, `capitalise`, `verbForm`, `countOfTotal`).

**"In prose" is the load-bearing half of that rule, and the owner narrowed it to exactly that on 2026-09-14.** A bare figure standing in a column or a heading is not a sentence and is not covered. Two positions live outside it: `· N` on a results section heading — it counts the cards below it rather than the market, and carries the partition guarantee — and **the landing page's per-city count, restored on 2026-09-14** after a week as *"svi registrirani"*. The count is the fact a reader wants from a coverage list, and withholding it to spare the smaller cities a size comparison was a judgment made for the reader rather than by them. It renders through `providerCountLabel` in `web/lib/copy.ts`, which brings back the **Croatian numeral-agreement rule** the deleted helpers encoded: *pogrebnik* for counts ending in 1 except 11, *pogrebnika* otherwise. That collapses to two branches only because this noun's genitive singular and genitive plural coincide — `grad` would need the third branch back (`grada` for 2–4, `gradova` for 5+), so don't generalise it to another noun without one. Covered by `web/lib/copy.test.ts`, since the live data exercises only one branch.

**The cost page shipped on 2026-09-14/15 and is the first page on the site to state prices** — `/koliko-kosta-pogreb`, with a four-question estimator in `web/lib/costs.ts`. Three rules govern it and all three were defects first. **Figures are published municipal tariffs, never any provider's price**, each carrying `source` and `asOf` — which is what keeps it inside the `Never` on pricing, since that rule forbids guessing *"without a verified source"*. **Every figure on screen is rounded to €10, and each total is the sum of the rounded parts beneath it** — round the levels independently and the headline disagrees with its own blocks. And **the grave plot is never priced at all**, because it is allocated by registration order rather than sold, so the page states the exclusion instead of guessing. The tariffs are Split's and **the page deliberately does not say so**; the reasoning, including why a city picker and a national average were both rejected, is in `SPEC_frontend.md` → The cost page. **Re-check the figures every January** — Split reprices on 1 January, and 2026 was a flat 5.00% uplift on every line.

Two promises died with it: the footer no longer says *"ne navodimo cijene"*, and `/nase-obecanje` no longer claims not to compare prices — it now says `Ne rangiramo po cijeni`, which keeps the independence claim that matters.

**`/koliko-kosta-pogreb` is the only price page the product will have, and that is now a rule** (owner decision, 2026-09-16). Two further cost pages were approved, one of them written in full, and both were cut: this is a directory with a cost page, not a price index. Items whose subject is a *market price* are out of the article pipeline; items whose subject is *procedure, rights or a recurring public fee* are in, figures included where the figure is what makes the rule legible. The same round produced a second rule — **check the query before writing, not the evidence file**: the dropped cremation page was built on the strongest fact in the research, while the term people actually search is `krematorij`, not its price. The live pipeline, with what is sourced and what is not, is `SPEC_frontend.md` → Planned content; the evidence is in `.research/RESEARCH_funeral_costs.md` (gitignored).

**`/preuzimanje-troskova-pogreba` shipped on 2026-09-16** — the statutory schemes that cover a funeral when the family does not. Two things about it are load-bearing. **It reads as an entitlement, never as charity**: the draft was titled *"Tko plaća pogreb ako obitelj ne može"*, which names the reader's failure in the headline, and the owner changed route and heading to name the mechanism instead. And **its sources are named inline, on each institution's name, rather than listed at the foot** the way `/sto-uciniti-prvo` does — every section here is a different scheme run by a different body, and the reader's next action is to contact that body. The veterans' caps are quoted exactly rather than rounded to €10, because a statutory cap is the entitlement itself and not a price.

**The masthead menu carries one level of grouping** as of 2026-09-16. `MENU` in `web/lib/nav.ts` holds links *and* groups; a group has children and deliberately **no `href`**, so a new page no longer has to choose between a fifth top-level slot and invisibility. Desktop opens a group in a `<details>`, mobile shows its children inline under a label, and the footer nests without any disclosure. `NavDisclosure` closes both masthead disclosures on navigation — on the route changing *and* on a link click, since choosing the page you are already on changes no pathname. It stays an enhancement: with no JavaScript everything still opens and closes on click.
