...
> Status: Phase 1 — **the database schema is complete and applied** to the hosted Supabase project, including `entities.slug`, Croatian `services.slug` values, and anonymous usage logging (`events` + `log_event`). **Seven cities and 51 providers are loaded** — Zagreb, Split, Rijeka, Zadar, Osijek, Pula, Dubrovnik (expanded 2026-09-03; Split's okolica filled in 2026-09-07, taking it from 7 providers to 13). The frontend is **live** — masthead, reading column and footer at every width, a rebuilt landing page, `/privatnost`, and a provider page carrying the product's only form — see [SPEC_frontend.md](SPEC_frontend.md).
